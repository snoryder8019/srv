...
import { buildDesignVars } from './plugins/designEngine.js';
import { localeMiddleware } from './middleware/locale.js';
import { buildLocalizedCopy } from './plugins/copyLocale.js';
...
import fieldPortalRouter from './routes/fieldPortal.js';
...
// ── Internal M2M endpoints (verified-domains for the VPS domain-provisioner) ──
// Mounted before resolveTenant: not tenant-scoped, guarded by SLAB_PROVISION_KEY.
...
// ── Design layer vendor mounts (see views/partials/_tokens.ejs) ─────────────
// open-props: framework-free CSS custom properties (easings/shadows/gradients).
app.use('/vendor/open-props', express.static(path.join(__dirname, 'node_modules/open-props'), { maxAge: '30d', immutable: true }));
// lucide: SVG sprite referenced via <use href="/vendor/lucide/sprite.svg#icon">.
app.use('/vendor/lucide', express.static(path.join(__dirname, 'node_modules/lucide-static'), { maxAge: '30d', immutable: true }));
// motion: UMD build → window.Motion { animate, inView, ... } for spring reveals.
app.use('/vendor/motion', express.static(path.join(__dirname, 'node_modules/motion/dist'), { maxAge: '30d', immutable: true }));
// Self-hosted variable fonts (woff2 in the packages; per-family CSS lives in
// public/fonts/*.css — generated by scripts/build-local-fonts.mjs).
app.use('/vendor/fonts', express.static(path.join(__dirname, 'node_modules/@fontsource-variable'), { maxAge: '365d', immutable: true }));
app.use('/vendor/fonts-static', express.static(path.join(__dirname, 'node_modules/@fontsource'), { maxAge: '365d', immutable: true }));
// gsap (+ ScrollTrigger/SplitText — free since 3.13) and lenis smooth scroll,
// consumed by views/partials/_cinema.ejs.
app.use('/vendor/gsap', express.static(path.join(__dirname, 'node_modules/gsap/dist'), { maxAge: '30d', immutable: true }));
app.use('/vendor/lenis', express.static(path.join(__dirname, 'node_modules/lenis/dist'), { maxAge: '30d', immutable: true }));
// Manifest of locally hosted families: partials/font-vars.ejs prefers these
// over fonts.googleapis.com (kills the third-party render-blocking request).
try {
  app.locals.localFonts = JSON.parse(fsSync.readFileSync(path.join(__dirname, 'config/local-fonts.json'), 'utf8'));
  app.locals.designEngine = buildDesignVars; // parametric tokens (plugins/designEngine.js)
} catch (e) {
  app.locals.localFonts = {};
  console.warn('[fonts] no config/local-fonts.json — falling back to Google Fonts everywhere');
}

// ── Tenant resolution — sets req.tenant, req.db, res.locals.brand ───────────
app.use(resolveTenant);

// ── Locale — sets res.locals.t / locale / htmlLang (after tenant so the tenant
// default language is available; before views so every render has t()) ────────
...
app.use('/field', fieldPortalRouter);

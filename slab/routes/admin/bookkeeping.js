...
router.get('/', async (req, res) => {
  ...
  const [invoices, clients] = await Promise.all([
    db.collection('invoices').find(filter).sort({ createdAt: -1 }).toArray(),
    db.collection('clients').find({}).project({ name: 1 }).toArray(),
  ]);
  ...
  res.render('admin/bookkeeping/index', {
    ...
    paypalConfigured: !!(req.tenant?.public?.paypalClientId && req.tenant?.secrets?.paypalSecret),
    stripeConfigured: !!req.tenant?.secrets?.stripeSecret,
  });
});
...
router.post('/:id/refund', async (req, res) => {
  try {
    ...
    if (provider === 'paypal') {
      refundResult = await refundCapture(transactionId, refundAmount, req.tenant);

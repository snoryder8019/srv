...
// Hardcoded admin credentials (for initial testing - should use DB later)
const ADMIN_USERS = {
  'admin': '$2b$10$8YQNw8Zr9nZ9X.XZx.XZxOqKqKqKqKqKqKqKqKqKqKqKqKqKqKq' // password: admin123
};
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    // TODO: Integrate with madladslab user database
...
// Get system stats
app.get('/api/system', authenticateJWT, async (req, res) => {
  try {
...
// Restart a service
app.post('/api/services/:serviceId/restart', authenticateJWT, async (req, res) => {
  try {
...
// Stop a service
app.post('/api/services/:serviceId/stop', authenticateJWT, async (req, res) => {
  try {
    const { serviceId } = req.params;
    const service = SERVICES.find(s => s.id === serviceId);
...
// Start a service
app.post('/api/services/:serviceId/start', authenticateJWT, async (req, res) => {
  try {
    const { serviceId } = req.params;
    const service = SERVICES.find(s => s.id === serviceId);
...
// Get service logs
app.get('/api/services/:serviceId/logs', authenticateJWT, async (req, res) => {
  try {
    const { serviceId } = req.params;
    const service = SERVICES.find(s => s.id === serviceId);

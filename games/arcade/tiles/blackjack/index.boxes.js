...
const blackjack = {
  ...
  _match(table) {
    if (!table._bj) {
      table._bj = { bankrolls: new Array(table.seatCount).fill(table.config.startChips), handNo: 0, shoe: null };
      table.scores = table._bj.bankrolls.slice();
    }
    return table._bj;
  },
  _occupied(table, s) { const x = table.seats[s]; return !!(x && (x.platformId || x.bot)); },
  _reservedFor(h, seat) { let r = 0; for (const b of h.boxes) if (b.owner === seat) r += boxReserved(b); return r; },
  ...
  _toDealer(table, h, events) {
    ...
    events.push({ type: 'settle', dealerTotal: dv.total, dealerBust, dealerBJ, dealerCards: h.dealer.slice(), breakdown, deltas, bankrolls: m.bankrolls.slice() });

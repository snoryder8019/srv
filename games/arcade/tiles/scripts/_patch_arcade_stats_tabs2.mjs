/**
 * v2: tab the arcade stats. Two independent exact-match replacements:
 *   (A) section markup euchre-only -> tabbed container
 *   (B) the loadArcadeStats() function body -> game-driven tabbed version
 *       (kept as its own contiguous block; the loadArcadeStats() *call* elsewhere
 *        is left as-is)
 * Adds renderStatsTabs() + STATS_GAMES inside the new function and self-invokes
 * renderStatsTabs() at the existing call site by appending after the body.
 */
import fs from 'fs';
...
// (A) section markup
const oldSection = `  <div class="games-header">ARCADE · EUCHRE</div>
...
const newSection = `  <div class="games-header">ARCADE · STATS</div>
...
if (s.split(oldSection).length - 1 !== 1) throw new Error('A: section anchor count != 1');
s = s.replace(oldSection, newSection);

// (B) function body — contiguous block from "async function loadArcadeStats() {"
// through its closing "  }" (the line before the blank line + loadActivity()).
const oldBody = `  async function loadArcadeStats() {
    try {
...
if (s.split(oldBody).length - 1 !== 1) throw new Error('B: function-body anchor count != 1');
s = s.replace(oldBody, newBody);

fs.writeFileSync(FILE, s);
console.log('arcade stats tabbed across euchre/hearts/dominoes/mahjong');

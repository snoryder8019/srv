...
import { startPersonas } from './personas.js';
...
const REAP_IDLE_MS = 60000;   // drop a table after this long with zero connected humans
async function tick(io, table) {
  const r = room(table.tableId);
  // Idle reaping: continuous casino tables (and any table) shouldn't linger once
  // every human has gone. Track last time a human was connected; reap after grace.
  const humansConnected = table.seats.filter((x) => x.platformId && !x.bot && x.connected).length;
  const now = Date.now();
  if (humansConnected > 0) { table._lastHumanAt = now; }
  else {
    if (table._lastHumanAt == null) table._lastHumanAt = now;
    if (now - table._lastHumanAt > REAP_IDLE_MS) {
      stopClock(table.tableId);
      try { walletSync.clear(table); } catch (e) {}   // drop stale wallet baseline so a re-created table re-seeds
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('table:join', async (payload = {}) => {
      try {
        ...
        const occ = table.seats[seat];
        if (!occ?.platformId || occ.bot) table.seatPlayer(seat, { ...identity });   // take empty OR bot-held seat

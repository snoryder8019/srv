...
class GalacticMap3D {
  ...
  handleServerPhysicsUpdate(data) {
    ...
    // Store latest physics data for connection checking
    ...
    if (data.galaxies && data.galaxies.length > 0) {
      data.galaxies.forEach(galaxyUpdate => {
        ...
        // DEBUG: Log what we receive for trail
        console.log(`🔍 Galaxy ${galaxyUpdate.id.substring(0, 8)} received data:`, {
          hasTrail: !!galaxyUpdate.trail,
          trailLength: galaxyUpdate.trail ? galaxyUpdate.trail.length : 0,
          keys: Object.keys(galaxyUpdate)
        });

        // Create/update purple orbital trail for galaxy (using actual position history from server)
        if (galaxyUpdate.trail && galaxyUpdate.trail.length > 0) {
          console.log(`💜 Galaxy ${galaxyUpdate.id.substring(0, 8)}: Trail with ${galaxyUpdate.trail.length} positions`);
          this.createGalaxyTrail(mesh, galaxyUpdate.id, galaxyUpdate.trail);
        } else {
          console.warn(`⚠️ Galaxy ${galaxyUpdate.id.substring(0, 8)}: No trail data yet`);
        }
      });
    }
    ...
    if (data.connections) {
      console.log(`📡 Received ${data.connections.length} connections from server`);
      this.updateConnectionsFromServer(data.connections);
    } else {
      console.warn('⚠️ No connections in physics update data');
    }
  ...
  updateConnectionsFromServer(connections) {
    ...
    if (this.currentLevel !== 'galactic') {
      console.log('⚠️ Skipping connections render - not in galactic view (currentLevel:', this.currentLevel, ')');
      return;
    }

    console.log(`🔗 Rendering ${connections.length} connection lines in galactic view`);

    // Clear existing connections
    const oldCount = this.connectionsGroup.children.length;
    ...
    console.log(`   Cleared ${oldCount} old connection lines`);

    // Create new connections
    connections.forEach((conn, index) => {
      if (index < 3) {
        console.log(`   Connection ${index}: ${conn.from} → ${conn.to}, state: ${conn.state}, distance: ${conn.distance}`);
      }
      this.createConnection(conn);
    });
    console.log(`   ✅ Created ${this.connectionsGroup.children.length} connection line objects`);
  }
  ...
  createConnection(conn) {
    ...
    // Use server-provided color for gradient (yellow→orange→red)
    let material;
    if (conn.state === 'stable') {
      // Green solid line for stable connections
      material = new THREE.LineBasicMaterial({
        color: conn.color || 0x00ff00,
        transparent: true,
        opacity: 0.8,
        linewidth: conn.isPrimary ? 3 : 2
      });
    } else if (conn.state === 'breaking') {
      // Yellow→Orange→Red solid (gradient based on distance)
      material = new THREE.LineBasicMaterial({
        color: conn.color || 0xff4400, // Use server color for gradient
        transparent: true,
        opacity: 0.75,
        linewidth: 2
      });
    } else if (conn.state === 'broken') {
      // Red dashed for recently broken connections
      material = new THREE.LineDashedMaterial({
        color: conn.color || 0xff0000,
        transparent: true,
        opacity: 0.5,
        linewidth: 2,
        dashSize: 20,
      ...
      // Blue dashed for forming connections
      material = new THREE.LineDashedMaterial({
        color: conn.color || 0x0088ff,
        transparent: true,
        opacity: 0.6,
        linewidth: 2,
        dashSize: 30,
    ...
    if (conn.state === 'forming' || conn.state === 'broken') {
  ...
  createGalaxyTrail(galaxyMesh, galaxyId, trailHistory = null) {
    ...
    if (this.currentLevel !== 'galactic') {
      console.log(`⚠️ Skipping trail for ${galaxyId.substring(0, 8)} - not in galactic view`);
      return;
    }

    const asset = this.assets.get(galaxyId);
    if (!asset || !galaxyMesh) {
      console.warn(`⚠️ Cannot create trail: asset or mesh missing for ${galaxyId.substring(0, 8)}`);
      return;
    }
    ...
    if (!trailHistory || trailHistory.length < 2) {
      ...
      console.log(`⚠️ Trail for ${galaxyId.substring(0, 8)}: Not enough history (${trailHistory ? trailHistory.length : 0} positions)`);
      return;
    }

    console.log(`✨ Creating trail for ${galaxyId.substring(0, 8)}: ${trailHistory.length} positions`);
    ...
    if (this.currentLevel === 'galaxy' && this.planetOrbits && this.planetOrbits.length > 0) {
      this.planetOrbits.forEach(orbit => {
        ...
        if (orbit.trail && orbit.trailPoints) {
          ...
          const trailLength = Math.PI; // Half orbit trail (180 degrees)
   ...
   */
  /**
   * Start animated travel along connection line
   */
  startConnectionTravel(fromAssetId, toAssetId, startPos, endPos, travelTimeSeconds) {
    console.log('🚀 Starting connection travel animation', {
      from: fromAssetId,
      to: toAssetId,
      travelTime: travelTimeSeconds
    });

    // Create character marker if it doesn't exist
    if (!this.travelingCharacterMarker) {
      ...
      this.travelingCharacterMarker = new THREE.Mesh(geometry, material);
      this.scene.add(this.travelingCharacterMarker);
    }

    // Set starting position
    this.travelingCharacterMarker.position.set(startPos.x, startPos.y, startPos.z || 0);
    this.travelingCharacterMarker.visible = true;

    // Store travel data
    this.activeTravelData = {
      startTime: Date.now(),
      duration: travelTimeSeconds * 1000, // Convert to milliseconds
      startPos: new THREE.Vector3(startPos.x, startPos.y, startPos.z || 0),
      endPos: new THREE.Vector3(endPos.x, endPos.y, endPos.z || 0),
      fromAssetId,
      toAssetId
    };

    // Show travel status notification
    const destinationAsset = this.assets.get(toAssetId);
    ...
    // Update character navigation status to in-transit
    this.setCharacterTravelStatus(fromAssetId, toAssetId, travelTimeSeconds);

    // Deduct fuel at start of travel
    this.deductTravelFuel(startPos, endPos);

    console.log('✅ Travel animation started - marker visible at starting position');
  }

  /**
   * Set character travel status to in-transit
   */
  async setCharacterTravelStatus(fromAssetId, toAssetId, travelTimeSeconds) {
    try {
      ...
      const result = await response.json();
      if (result.success) {
        console.log('✅ Character travel status set to in-transit');
        ...
        console.error('❌ Failed to set travel status:', result.error);
      }
    } catch (error) {
      console.error('❌ Error setting travel status:', error);
    }
  }

  /**
   * Clear character travel status (set to docked/not in transit)
   */
  async clearCharacterTravelStatus() {
    try {
      ...
      const result = await response.json();
      if (result.success) {
        console.log('✅ Character travel status cleared (now docked)');

        // Update local character data
        if (window.currentCharacter && window.currentCharacter.navigation) {
        ...
        console.error('❌ Failed to clear travel status:', result.error);
      }
    } catch (error) {
      console.error('❌ Error clearing travel status:', error);
    }
  }
   ...
   */
  async deductTravelFuel(startPos, endPos) {
    try {
      const dx = endPos.x - startPos.x;
      const dy = endPos.y - startPos.y;
      const dz = (endPos.z || 0) - (startPos.z || 0);
      const distance = Math.sqrt(dx*dx + dy*dy + dz*dz);

      const fuelRequired = Math.ceil(distance * 0.5);

      console.log(`⛽ Deducting ${fuelRequired} fuel for ${Math.round(distance)} unit journey`);

      const characterId = window.currentCharacter?._id;
      if (!characterId) return;

      const response = await fetch('/api/v1/travel/deduct-fuel', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          characterId,
      ...
      const result = await response.json();
      if (result.success) {
        console.log(`✅ Fuel deducted: ${fuelRequired} units`);
        // Update local character data
        if (window.currentCharacter?.ship?.fittings?.fuelTanks) {
          window.currentCharacter.ship.fittings.fuelTanks.remaining -= fuelRequired;
        }
      } else {
        console.error('❌ Failed to deduct fuel:', result.error);
      }
    } catch (error) {
      console.error('❌ Error deducting fuel:', error);
    }
  }

  /**
   * Update traveling character position (called in animation loop)
   * Follows the actual connection line path dynamically
   */
  updateTravelingCharacter() {
    if (!this.activeTravelData || !this.travelingCharacterMarker) return;

    const now = Date.now();
    const elapsed = now - this.activeTravelData.startTime;
    const progress = Math.min(elapsed / this.activeTravelData.duration, 1.0);

    // Get current positions of source and destination assets (they may have moved!)
    ...
    if (!fromAsset || !toAsset) {
      console.error('❌ Travel assets not found');
      return;
    }

    // Get current positions from the actual mesh positions (updated by physics)
    ...
    // Lerp along the CURRENT path (which updates as galaxies move)
    ...
    // Optional: Make orb look at destination
    this.travelingCharacterMarker.lookAt(toPos);

    // Update travel status bar with progress
    const remaining = this.activeTravelData.duration - elapsed;
    const etaSeconds = Math.ceil(remaining / 1000);
    const percent = progress * 100;
    const destinationName = toAsset.title || toAsset.name || 'Unknown';

    if (window.updateTravelProgress) {
      window.updateTravelProgress(percent, etaSeconds, destinationName);
    }

    // Arrival check
    if (progress >= 1.0) {
      console.log('✅ Travel complete! Arrived at destination');
      this.travelingCharacterMarker.visible = false;

      // Update character location on server
      this.completeTravelToDestination(this.activeTravelData.toAssetId);

      this.activeTravelData = null;
    }
  }
   ...
   */
  async completeTravelToDestination(destinationAssetId) {
    console.log('📍 Updating character location to:', destinationAssetId);

    try {
      const characterId = window.currentCharacter?._id;
      if (!characterId) {
        console.error('❌ No current character ID found');
        return;
      }
      ...
      if (result.success) {
        ...
        if (window.socket) {
          window.socket.emit('characterMoved', {
            characterId: characterId,
            characterName: window.currentCharacter?.name,
            userId: window.currentCharacter?.userId,
            location: window.currentCharacter.location,
            assetId: destinationAssetId
          });
          console.log('📡 Socket event emitted: characterMoved');
        }

        // Clear in-transit status (set to docked)
        ...
        if (window.showTravelStatus) {
          window.showTravelStatus(
            ...
            true // isArrival = true (green color)
          );

          // Hide status bar after 4 seconds
          setTimeout(() => {
            if (window.hideTravelStatus) {
              window.hideTravelStatus();
            }
          }, 4000);
        }

        // Also show legacy notification if available
        if (window.showNotification) {
          window.showNotification(`✅ Arrived at ${destinationName}`, 'success');
        }

      } else {
        console.error('❌ Failed to update character location:', result.error);
      }
    } catch (error) {

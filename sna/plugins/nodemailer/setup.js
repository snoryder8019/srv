...
export const sendDynamicEmail = async (to, emailType, user, dynamicLink) => {
  ...
  const settings = {
    ...
    general: {
      subject: 'Some News Article sent you a message',
...
export const emailOutGeneral = async (req, res) => {
  try {
    ...
    const htmlTemplate = fs
      .readFileSync(templatePath, 'utf8')
      .replace('{dynamicBody}', emailBody)
      .replace('{dynamicLink}', 'https://somenewsarticle.com')
      .replace(
        '{unsubscribeLink}',
        'https://somenewsarticle.com/unsubscribe'
    ...
    const mailOptions = {
      ...
      subject: 'Message from Some News Article', // Email subject

...
router.get('/:token', async (req, res) => {
  try {
    const db = getDb();
    const meeting = await db.collection('w2_meetings').findOne({
...
router.get('/:token/qr', async (req, res) => {
  try {
    const url = `${config.DOMAIN}/meeting/${req.params.token}`;
...
router.post('/:token/invite', express.json(), async (req, res) => {
  try {
    ...
    const meetingUrl = `${config.DOMAIN}/meeting/${meeting.token}`;
...
router.post('/:token/upload', express.json(), meetingAssetUpload.single('file'), async (req, res) => {
  try {
    const db = getDb();
    const meeting = await db.collection('w2_meetings').findOne({ token: req.params.token, status: 'active' });
    if (!meeting) return res.status(404).json({ error: 'Meeting not found' });
...
router.get('/:token/data', async (req, res) => {
  try {
    const db = getDb();
    const meeting = await db.collection('w2_meetings').findOne({ token: req.params.token });

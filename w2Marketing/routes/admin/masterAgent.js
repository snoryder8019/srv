...
const WORKFLOW_PROMPT = `You are a workflow planner for a marketing agency admin panel.
...
function getSuggestions(department, task, fill) {
  ...
  const s = {
    ...
    copy: [
      ...
      `Email a client about our updated services`,
    ],
    section: [
      `Write a blog post about "${topic}"`,
    ...
    page: [
      ...
      `Draft an invoice for web design services`,
    ],
    design: [
      `Switch to a modern blue and coral color palette`,
      `Show the blog section on the homepage`,
      `Create a social graphic with the new brand colors`,
    ],
    asset: [
      `Create an Instagram story promoting our services`,
      `Design a Facebook cover with our brand colors`,
...
// Master suggestion pool — single-action items for the recycle button
const SUGGESTION_POOL = [
  // Content creation
  'Write a blog post about social media tips for local businesses',
  'Refresh the hero headline with something compelling',
  'Add an FAQ section about our services and pricing',
  'Write a CTA banner for booking a free consultation',
  'Create a landing page for our top service',
  'Add a cards section highlighting key features',
  // Email & outreach
  'Draft an email campaign for this month\'s newsletter',
  ...
  'Write a follow-up email for a recent proposal',
  // Business
  'Draft an invoice for web design services',
  'Create an invoice for monthly social media management',
  // Design & assets
  'Create an Instagram post promoting our services',
  'Design a Facebook cover with our brand colors',
...
function getRandomSuggestions(n = 4, exclude = []) {
...
router.get('/suggestions', (_req, res) => {
  res.json({ suggestions: getRandomSuggestions(4) });
});
...
router.post('/run-task', async (req, res) => {
  ...
  try {
    ...
    if (department === 'blog') {
      ...
      toolArgs = { topic: task, context };
    } else if (department === 'section') {
      toolName = 'fill_section';
      toolArgs = { section_type: section_type || 'text', task, context };
    } else if (department === 'page') {
      toolName = 'write_page';
      toolArgs = { title: task, page_type: page_type || 'content', task, context };
    } else if (department === 'design') {
      toolName = 'update_design';
      toolArgs = { task, context };
    } else if (department === 'asset') {
      toolName = 'generate_social_image';
      toolArgs = { prompt: task, context };
      ...
      toolArgs = { task, section: 'all', context };
    }

    const result = await runTool(toolName, toolArgs);
    const action = DEPT_ACTIONS[department] || DEPT_ACTIONS.copy;
    const suggestions = getSuggestions(department, task, result.fill);
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    if (route.department === 'blog') {
      ...
      toolName = 'update_design';
      toolArgs = { task: route.task, context };
    } else if (route.department === 'asset') {
      toolName = 'generate_social_image';
      toolArgs = { prompt: route.task, context };
...
router.post('/execute', async (req, res) => {
  ...
  try {
    ...
    if (department === 'copy') {
      ...
      const s3key = `${DIR_PREFIX}/assets/${folder}/${filename}`;
      ...
      const insertResult = await db.collection('w2_assets').insertOne(assetDoc);
      ...
      const result = await db.collection('w2_invoices').insertOne({
        clientId: null,
        invoiceNumber,
        title,
        lineItems: items,

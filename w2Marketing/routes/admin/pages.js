...
const ALLOWED_COLLECTIONS = ['w2_blog', 'w2_portfolio'];
...
function pageFields(body, current) {
  ...
  const dataCollection = ALLOWED_COLLECTIONS.includes(body.dataCollection) ? body.dataCollection : 'w2_blog';
...
router.get('/', async (req, res) => {
  try {
    const db = getDb();
    const pages = await db.collection('w2_pages').find({}).sort({ createdAt: -1 }).toArray();
...
router.post('/agent', async (req, res) => {
  ...
  try {
    ...
    let systemPrompt;
    if (isLanding) {
      systemPrompt = `You are a web page builder assistant for W2 Marketing, a digital marketing agency in Greeley, Colorado.
...
Tone: professional, local, trustworthy. Audience: small businesses in Greeley CO.${pageCtx}${researchCtx}`;
    } else {
...
- End with a mention of W2 Marketing in Greeley CO
Tone: professional, approachable. Audience: small businesses in Northern Colorado.${pageCtx}${researchCtx}`;
    }
...
router.post('/', async (req, res) => {
  try {
    ...
    const existing = await db.collection('w2_pages').findOne({ slug: finalSlug });
    if (existing) {
      return res.render('admin/pages/form', {
        user: req.adminUser, page: 'pages', title: 'New Page', pg: req.body,
    ...
    await db.collection('w2_pages').insertOne({ title, slug: finalSlug, ...fields, createdAt: now });
...
router.get('/:id/edit', async (req, res) => {
  try {
    const db = getDb();
    const pg = await db.collection('w2_pages').findOne({ _id: new ObjectId(req.params.id) });
...
router.post('/:id', async (req, res) => {
  try {
    ...
    const existing = await db.collection('w2_pages').findOne({
    ...
    const current = await db.collection('w2_pages').findOne({ _id: new ObjectId(req.params.id) });
    const fields = pageFields(req.body, current);
    await db.collection('w2_pages').updateOne(
...
router.post('/:id/delete', async (req, res) => {
  try {
    const db = getDb();
    await db.collection('w2_pages').deleteOne({ _id: new ObjectId(req.params.id) });

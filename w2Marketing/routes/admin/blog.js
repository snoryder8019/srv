...
router.get('/', async (req, res) => {
  try {
    const db = getDb();
    const posts = await db.collection('w2_blog').find({}).sort({ createdAt: -1 }).toArray();
...
router.post('/', async (req, res) => {
  try {
    ...
    const existing = await db.collection('w2_blog').findOne({ slug: finalSlug });
    if (existing) {
    ...
    const now = new Date();
    await db.collection('w2_blog').insertOne({
      title,
      slug: finalSlug,
      excerpt: excerpt || '',
      content: content || '',
...
router.get('/:id/edit', async (req, res) => {
  try {
    const db = getDb();
    const post = await db.collection('w2_blog').findOne({ _id: new ObjectId(req.params.id) });
...
- End with a soft call-to-action mentioning W2 Marketing
...
router.post('/:id', async (req, res) => {
  try {
    ...
    const { title, slug, excerpt, content, category, tags, status, featuredImageUrl } = req.body;
    const finalSlug = slug ? toSlug(slug) : toSlug(title);
    const existing = await db.collection('w2_blog').findOne({
      slug: finalSlug,
    ...
    const current = await db.collection('w2_blog').findOne({ _id: new ObjectId(req.params.id) });
    const now = new Date();
...
router.post('/:id/delete', async (req, res) => {
  try {
    const db = getDb();
    await db.collection('w2_blog').deleteOne({ _id: new ObjectId(req.params.id) });

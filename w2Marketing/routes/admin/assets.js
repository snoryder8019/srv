...
router.post('/generate-bg', express.json(), async (req, res) => {
  try {
    ...
    const { key, url } = await uploadToLinode(pngBuffer, 'ai-backgrounds', `sd-bg-${Date.now()}.png`, 'image/png');
...
router.post('/agent', express.json({ limit: '2mb' }), async (req, res) => {
  try {
    ...
    if (isImageRequest) {
      ...
      const result = await runTool('generate_social_image', { prompt: lastMsg });
      ...
      for (let i = 0; i < sdBuffers.length; i++) {
        ...
        let sdBgUrl = null;
        if (sdBuffers[i]) {
          const sdUpload = await uploadToLinode(sdBuffers[i], 'ai-backgrounds', `sd-bg-${Date.now()}-${i}.png`, 'image/png');
          sdBgUrl = sdUpload.url;
        }

        // Upload composite preview
        const suffix = variationCount > 1 ? `-v${i + 1}` : '';
        const { key, url, filename } = await uploadToLinode(pngBuffer, folder, `${title.replace(/\s+/g, '-')}${suffix}.png`, 'image/png');
        ...
        const inserted = await db.collection('w2_assets').insertOne(doc);
      ...
      const result = await runTool('manage_assets', { action: lastMsg, query: lastMsg });
      ...
      if (planned.action === 'list' || planned.action === 'search') {
        ...
        const assets = await db.collection('w2_assets').find(query).sort({ uploadedAt: -1 }).limit(20).toArray();
        ...
        if (name) {
          ...
          const existing = await db.collection('w2_asset_folders').findOne({ slug });
          if (!existing) {
            await db.collection('w2_asset_folders').insertOne({ name, slug, createdAt: new Date() });
...
router.get('/export', async (req, res) => {
  try {
    ...
    const assets = await db.collection('w2_assets').find(query).sort({ uploadedAt: -1 }).toArray();
...
router.get('/clients', async (req, res) => {
  try {
    const db = getDb();
    const clients = await db.collection('w2_clients')
...
router.get('/folders', async (req, res) => {
  try {
    const db = getDb();
    const folders = await db.collection('w2_asset_folders').find({}).sort({ name: 1 }).toArray();
    res.json({ folders });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.post('/folders', express.json(), async (req, res) => {
  try {
    ...
    const r = await db.collection('w2_asset_folders').insertOne(doc);
...
router.put('/folders/:id', express.json(), async (req, res) => {
  try {
    const db = getDb();
    const folder = await db.collection('w2_asset_folders').findOne({ _id: new ObjectId(req.params.id) });
    ...
    const dup = await db.collection('w2_asset_folders').findOne({ slug: newSlug, _id: { $ne: folder._id } });
    if (dup) return res.status(400).json({ error: 'A folder with that name already exists' });
    const oldSlug = folder.slug;
    await db.collection('w2_asset_folders').updateOne({ _id: folder._id }, { $set: { name, slug: newSlug, updatedAt: new Date() } });
    // Update all assets that reference the old slug
    if (oldSlug !== newSlug) {
      // Update `folders` array entries
      await db.collection('w2_assets').updateMany(
        { folders: oldSlug },
        { $set: { 'folders.$': newSlug } }
      );
...
router.delete('/folders/:id', async (req, res) => {
  try {
    ...
    const soloAssets = await db.collection('w2_assets').find({
      $or: [
    ...
    const legacySolo = await db.collection('w2_assets').find({
      folder: slug,
    ...
    if (soloIds.length) {
      await db.collection('w2_assets').deleteMany({ _id: { $in: soloIds } });
    }
    ...
    await db.collection('w2_asset_folders').deleteOne({ _id: folder._id });
...
router.get('/social/presets', async (req, res) => {
  try {
    ...
    const presets = await db.collection('w2_social_presets')
...
router.post('/social/presets', express.json({ limit: '5mb' }), async (req, res) => {
  try {
    ...
    const r = await db.collection('w2_social_presets').insertOne(doc);
...
router.get('/social/presets/:id', async (req, res) => {
  try {
    const db = getDb();
    const preset = await db.collection('w2_social_presets').findOne({ _id: new ObjectId(req.params.id) });
...
router.put('/social/presets/:id', express.json({ limit: '5mb' }), async (req, res) => {
  try {
    ...
    await db.collection('w2_social_presets').updateOne({ _id: new ObjectId(req.params.id) }, { $set });
...
router.delete('/social/presets/:id', async (req, res) => {
  try {
    const db = getDb();
    await db.collection('w2_social_presets').deleteOne({ _id: new ObjectId(req.params.id) });
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.get('/list', async (req, res) => {
  try {
    ...
    const [assets, total] = await Promise.all([
      db.collection('w2_assets').find(query).sort(sortObj).skip(Number(skip)).limit(Number(limit)).toArray(),
...
router.post('/social-upload', assetMem.single('image'), async (req, res) => {
  try {
    ...
    const { key, url, filename } = await uploadToLinode(req.file.buffer, folder, name, 'image/png');
    const doc = {
      filename,
      originalName: name,
      folders: [folder],
...
router.post('/bulk-delete', async (req, res) => {
  try {
    ...
    const assets = await db.collection('w2_assets').find({ _id: { $in: objectIds } }).toArray();
    ...
    await db.collection('w2_assets').deleteMany({ _id: { $in: objectIds } });
...
router.post('/bulk-move', async (req, res) => {
  try {
    ...
    await db.collection('w2_assets').updateMany({ _id: { $in: objectIds } }, { $set });
...
router.post('/:id/share', async (req, res) => {
  try {
    ...
    await db.collection('w2_assets').updateOne({ _id: asset._id }, { $set: { shareToken: token, sharedAt: new Date() } });
...
router.delete('/:id/share', async (req, res) => {
  try {
    const db = getDb();
    await db.collection('w2_assets').updateOne(

...
router.get('/', async (req, res) => {
  ...
  const [contacts, campaigns, clients, campaignEvents] = await Promise.all([
    db.collection('w2_contacts').find({}).sort({ createdAt: -1 }).toArray(),
    db.collection('w2_campaigns').find({}).sort({ createdAt: -1 }).toArray(),
    db.collection('w2_clients').find({}).project({ name: 1, email: 1, status: 1 }).toArray(),
    db.collection('w2_campaign_events').aggregate([
...
router.post('/contacts', async (req, res) => {
  try {
    ...
    const existing = await db.collection('w2_contacts').findOne({ email: email.toLowerCase().trim() });
    if (existing) return res.redirect('/admin/email-marketing?tab=contacts&error=Contact+already+exists');

    await db.collection('w2_contacts').insertOne({
...
router.post('/contacts/upload', csvUpload.single('csv'), async (req, res) => {
  try {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const existing = await db.collection('w2_contacts').findOne({ email });
...
router.post('/contacts/import-clients', async (req, res) => {
  try {
    const db = getDb();
    const clients = await db.collection('w2_clients').find({ email: { $exists: true, $ne: '' } }).toArray();
    let imported = 0, skipped = 0;
...
router.post('/contacts/:id/update', async (req, res) => {
  try {
    ...
    await db.collection('w2_contacts').updateOne({ _id: new ObjectId(req.params.id) }, { $set: update });
...
router.post('/contacts/:id/delete', async (req, res) => {
  const db = getDb();
  await db.collection('w2_contacts').deleteOne({ _id: new ObjectId(req.params.id) });
...
router.post('/campaigns', async (req, res) => {
  try {
    ...
    await db.collection('w2_campaigns').insertOne({
      subject,
      preheader: preheader || '',
      body,
...
router.post('/campaigns/agent', async (req, res) => {
  ...
  try {
    ...
    const [contactCount, design] = await Promise.all([
      db.collection('w2_contacts').countDocuments({ status: 'subscribed' }),
      db.collection('w2_design').findOne({}),
    ]);
    const agentName = design?.agent_name || 'W2 Marketing';
    ...
    const systemPrompt = `You are a marketing email writing assistant for ${agentName}, a digital marketing agency in Greeley, Colorado.
...
- Include a clear call-to-action with a styled button: <a href="https://w2marketing.biz" style="display:inline-block;padding:12px 28px;background:#C9A848;color:#1C2B4A;text-decoration:none;font-weight:600;border-radius:3px;">Button Text</a>
...
Audience: small business owners in Greeley / Northern Colorado.
...
router.post('/campaigns/:id/send', async (req, res) => {
  try {
    const db = getDb();
    const campaign = await db.collection('w2_campaigns').findOne({ _id: new ObjectId(req.params.id) });
    ...
    if (campaign.targetContactIds?.length) {
      contacts = await db.collection('w2_contacts').find({
      ...
      contacts = await db.collection('w2_contacts').find(contactFilter).toArray();
    }
    ...
    for (const contact of contacts) {
      try {
        await sendCampaignEmail(contact.email, contact.name, campaign.subject, campaign.preheader, campaign.body, campaign._id, contact._id);
        sent++;
      } catch (err) {
    ...
    await db.collection('w2_campaigns').updateOne(
      { _id: campaign._id },
...
router.get('/campaigns/:id', async (req, res) => {
  try {
    ...
    const events = await db.collection('w2_campaign_events')
    ...
    const contacts = await db.collection('w2_contacts').find(contactFilter).toArray();
    ...
    const otherCampaigns = await db.collection('w2_campaigns')
...
router.post('/campaigns/:id/follow-up', async (req, res) => {
  try {
    const db = getDb();
    const parentCampaign = await db.collection('w2_campaigns').findOne({ _id: new ObjectId(req.params.id) });
    ...
    const events = await db.collection('w2_campaign_events').find({ campaignId: parentCampaign._id }).toArray();
    ...
    const allContacts = await db.collection('w2_contacts').find(contactFilter).toArray();
    ...
    const result = await db.collection('w2_campaigns').insertOne({
      subject,
      preheader: preheader || '',
      body,
...
router.post('/campaigns/:id/delete', async (req, res) => {
  const db = getDb();
  await db.collection('w2_campaigns').deleteOne({ _id: new ObjectId(req.params.id) });

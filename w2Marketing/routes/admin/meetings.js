...
router.get('/', async (req, res) => {
  try {
    ...
    await db.collection('w2_meetings').updateMany(
      { status: 'active', expiresAt: { $lt: now } },
      { $set: { status: 'expired' } }
    );

    const meetings = await db.collection('w2_meetings')
      .find()
      .sort({ createdAt: -1 })
      .toArray();

    // Fetch clients and users for tag selection + resolution
    const clients = await db.collection('w2_clients').find({}, { projection: { name: 1, email: 1, company: 1 } }).sort({ name: 1 }).toArray();
...
router.delete('/:id/destroy', async (req, res) => {
  try {
    const db = getDb();
    await db.collection('w2_meetings').deleteOne({ _id: new ObjectId(req.params.id) });
    res.json({ ok: true });
  } catch (err) {
...
router.delete('/:id', async (req, res) => {
  try {
    const db = getDb();
    await db.collection('w2_meetings').updateOne(
...
router.get('/:id', async (req, res) => {
  try {
    const db = getDb();
    const meeting = await db.collection('w2_meetings').findOne({ _id: new ObjectId(req.params.id) });
    if (!meeting) return res.status(404).send('Meeting not found');

    // Resolve tagged clients and users
    const taggedClients = meeting.tags?.clients?.length
      ? await db.collection('w2_clients').find({ _id: { $in: meeting.tags.clients } }).toArray()
      : [];
    ...
    const allClients = await db.collection('w2_clients').find({}, { projection: { name: 1, email: 1, company: 1 } }).sort({ name: 1 }).toArray();

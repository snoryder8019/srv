...
async function findPayableInvoice(token) {
  const db = getDb();
  const invoice = await db.collection('w2_invoices').findOne({ paymentToken: token });
...
router.post('/:token/stripe', async (req, res) => {
  try {
    ...
    const session = await createCheckoutSession(
      invoice,
      clientDoc,
      `${config.DOMAIN}/pay/${req.params.token}/success?provider=stripe&session_id={CHECKOUT_SESSION_ID}`,
      `${config.DOMAIN}/pay/${req.params.token}?cancelled=1`
    );
...
router.post('/:token/paypal', async (req, res) => {
  try {
    ...
    const order = await createOrder(
      invoice,
      clientDoc,
      `${config.DOMAIN}/pay/${req.params.token}/success?provider=paypal`,
      `${config.DOMAIN}/pay/${req.params.token}?cancelled=1`
    );
...
router.get('/:token/success', async (req, res) => {
  try {
    const db = getDb();
    const invoice = await db.collection('w2_invoices').findOne({ paymentToken: req.params.token });

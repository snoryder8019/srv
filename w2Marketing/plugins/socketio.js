...
export function initSocketIO(server) {
  ...
  meetings.on('connection', (socket) => {
    ...
    try {
      ...
      const match = cookies.match(/w2_token=([^;]+)/);
      if (match) {
    ...
    socket.on('join-room', async (data) => {
      ...
      try {
        const db = getDb();
        const meeting = await db.collection('w2_meetings').findOne({ token, status: 'active' });
        ...
        if (meeting.expiresAt && new Date(meeting.expiresAt) < new Date()) {
          await db.collection('w2_meetings').updateOne({ _id: meeting._id }, { $set: { status: 'expired' } });
        ...
        if (!adminUser) {
          ...
          const matchedClient = await db.collection('w2_clients').findOne({
    ...
    socket.on('meeting-note-edit', async (data) => {
      ...
      try {
        ...
        let result = await db.collection('w2_meetings').updateOne(
    ...
    socket.on('transcription-chunk', async (data) => {
      ...
      try {
        const db = getDb();
        const meeting = await db.collection('w2_meetings').findOne({ token }, { projection: { title: 1 } });
  ...
  setTimeout(() => {
    try {
      const db = getDb();
      db.collection('w2_meetings').createIndex({ token: 1 }, { unique: true }).catch(() => {});
    } catch {}
  }, 5000);

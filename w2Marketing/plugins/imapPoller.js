...
async function resolveThread(db, clientId, subject, inReplyTo) {
  ...
  if (inReplyTo) {
    const parent = await db.collection('w2_client_emails').findOne({ messageId: inReplyTo, clientId });
    if (parent?.threadId) return parent.threadId;
  }

  // Fall back to matching by normalized subject + client
  const match = await db.collection('w2_client_emails').findOne(
  ...
  try {
    ...
    for (const uid of messages) {
      try {
        ...
        const existing = await db.collection('w2_client_emails').findOne({ messageId });
        ...
        const clientDoc = await db.collection('w2_clients').findOne({ email: from });
